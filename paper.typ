...
*Замыкание* #sym.dash.em это сущность, с которой связывается функция при формировании динамического окружения. По сути это результат определения функции.

*Замыкание состоит из следующих частей:* имени функции (при его наличии), её определения, параметров функции и копии динамического окружения в момент её определения.

Напимер, замыкание для функции f: (f, (f x = a `*` x, [(a, 3)])).

*Замыкание формируется* в момент определения функции в программе.

*Замыкание используется для* вычисления значения вызова функции. Само замыкание будет скопировано во временное динамическое окружение, формируемое во время вызова функции, после завершения вычислений, копия удаляется, размораживая при этом динамическое окружение и подставляя в него результат вычислений.
